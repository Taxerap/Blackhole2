...
*/
    ...
    html_content_size = html_head_size + html_file_size + 3;
    ...
    // Create data thread
    // It seems like after1 C17, initialize atomic variable like atomic_int gg = 1 is allowed,
    // but this should work as well. 
    atomic_store(&clients_mtx_locked, false);
    atomic_store(&new_client_incoming, false);
    atomic_store(&data_thread_done, false);
    atomic_store(&data_thread_done, false);
    ...
    // Not using conditional variable here, because it should be very quick...
    while (!atomic_load_explicit(&clients_mtx_locked, memory_order_acquire))
    ...
    while (!atomic_load(&should_exit))
    {
        ...
        {
            bh2_log_debug("[Main] Start to end program.");
        ...
        */
        ...
        bh2_log_info("[Main] Waiting for data thread to give the mutex.");
    ...
    bh2_log_trace("[Main] Main thread done by data thread signal.");
...
clean_multithread:
    ...
    free(html_content);
    bh2_log_info("[Main] Main has ended. End of log.");
#ifdef BH2_DEBUG
    if (has_log)
    {
        mtx_destroy(&log_mtx);
...
MainThreadSignalHandler( [[maybe_unused]] int signum_unused )
{
    ...
    bh2_log_debug("[Main] Caught SIGINT.");
